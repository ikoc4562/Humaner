...
$sql=$db->prepare ('select * from partners where id=:id');
...
if (@$_POST['isGuncelle'])
{
    if (@$_FILES['fotograf']['name']<>'')
    {
        ...
        $sql=$db->prepare('update partners set title=:title,isdeleted=:is_deleted,picture=:resim,link=:link where id=:id');
        $sql->execute(['title'=>$_POST['title'],'is_deleted'=>0,'id'=>$_GET['id'],'resim'=>$resim ,'link'=>$_POST['link']]);

    }
    else
    {
        $sql=$db->prepare('update partners set title=:title,isdeleted=:is_deleted,link=:link where id=:id');
        $sql->execute(['title'=>$_POST['title'],'is_deleted'=>0,'id'=>$_GET['id'] ,'link'=>$_POST['link']]);
