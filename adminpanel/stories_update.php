...
$sql=$db->prepare ('select * from stories where id=:id');
...
if (@$_POST['isGuncelle'])
{
    if (@$_FILES['fotograf']['name']<>'')
    {
        ...
        $sql=$db->prepare('update stories set title=:title,is_deleted=:is_deleted,picture=:resim,text=:text where id=:id');
        $sql->execute(['title'=>$_POST['title'],'is_deleted'=>0,'id'=>$_GET['id'],'resim'=>$resim ,'text'=>$_POST['text']]);

    }
    else
    {
        $sql=$db->prepare('update stories set title=:title,is_deleted=:is_deleted,text=:text where id=:id');
        $sql->execute(['title'=>$_POST['title'],'is_deleted'=>0,'id'=>$_GET['id'] ,'text'=>$_POST['text']]);
          ...
          <form method="post" enctype="multipart/form-data">
              ...
              <div class="box-body">
                  ...
                  <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="text" value="<?=$events[0]['text']?>">
                  </div>
                  <div class="form-group">
